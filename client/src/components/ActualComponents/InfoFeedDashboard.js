...
const InfoFeedDashboard = ({ selectedExercise ,isMobileView, handleToggleSidebar,closeInfoFeed2}) => {
    const exercise = SidebarData[selectedExercise];
    const [targetValue, setTargetValue] = useState(0);
    const [achievedValue, setAchievedValue] = useState(0);
    const [timeTaken, setTimeTaken] = useState(0);

    const handleTargetChange = (target, achieved, time) => {
        setTargetValue(target);
        setAchievedValue(achieved);
        setTimeTaken(time);
    ...
    return (
        <div className="">
            ...
            <SetTargetBox handleToggleSidebar = {handleToggleSidebar} exerciseId={selectedExercise + 1} onTargetChange={handleTargetChange} />
